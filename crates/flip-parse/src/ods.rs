...
pub fn parse(path: &Path) -> Result<Document> {
    ...
    doc.metadata = Metadata {
        source_format: Some("ods".to_string()),
        ..Default::default()
    };
    ...
    if let Ok(mut file) = zip.by_name("content.xml") {
        ...
        if file.read_to_string(&mut content).is_ok() {
            let text = extract_ods_text(&content);
    ...
    if doc.blocks.is_empty() {
        doc.push_block(Block::paragraph(
            "(No text content could be extracted from this ODS spreadsheet)",
        ));
    }
...
fn extract_ods_text(xml: &str) -> String {
    ...
    for ch in xml.chars() {
        match ch {
            '<' => {
                in_tag = true;
                if in_value && !current.trim().is_empty() {
